...
params = {
    'start_time': '20220407000000',
    'end_time': '20220408000000'
}

res = requests.get('http://192.168.2.9:9999/v1/device/photo/snap_count/?format=json', params)
results = res.json()
...
if end_date - start_date <= days:
    ...
    vehicle_hour_list = [{'date': ''.join([str(i), '点']), 'count': 0} for i in range(24)]
    ...
    people_hour_list = [{'date': (start_date + datetime.timedelta(days=i)).strftime('%Y-%m-%d'), 'count': 0} for i in range(dlt_day)]
    vehicle_hour_list = [{'date': (start_date + datetime.timedelta(days=i)).strftime('%Y-%m-%d'), 'count': 0} for i in range(dlt_day)]
