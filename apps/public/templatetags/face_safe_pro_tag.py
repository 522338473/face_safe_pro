...
def device_status():
    ...
    :return: 摄像头状态
    """
    return device_model.DeviceInfo.objects.values('status').annotate(count=Count('status')).values('status', 'count')
...
def device_info(*args, **kwargs):
    ...
    device_page = Paginator(device_model.DeviceInfo.objects.values('name', 'ip', 'status', 'address', 'geo').order_by('id'), page_size)
    if device_page.page_range.start <= current_page < device_page.page_range.stop:
        return {'device_page': device_page, 'device_list': device_page.page(current_page).object_list}
...
def alarm_count():
    ...
    """
    monitor_discovery_total = monitor_model.MonitorDiscover.objects.filter(target__area=False).count()
    vehicle_discovery_total = monitor_model.VehicleMonitorDiscover.objects.count()
    area_discovery_total = monitor_model.MonitorDiscover.objects.filter(target__area=True).count()
...
def alarm_unchecked_count():
    ...
    area_discovery_total: 区域报警
    """
    monitor_discovery_total = monitor_model.MonitorDiscover.objects.filter(target__area=False, checked=False).count()
    vehicle_discovery_total = monitor_model.VehicleMonitorDiscover.objects.filter(checked=False).count()
    area_discovery_total = monitor_model.MonitorDiscover.objects.filter(target__area=True, checked=False).count()
...
def alarm_list(*args, **kwargs):
    ...
    current_page = kwargs.get('current_page', 1)
    page_size = kwargs.get('page_size', 10)
    alarm_page = Paginator(monitor_model.MonitorDiscover.objects.order_by('record__take_photo_time').values('id', 'target__name', 'target__photo', 'record__head_path'), page_size)
    if alarm_page.page_range.start <= current_page < alarm_page.page_range.stop:
        return {'alarm_page': alarm_page, 'alarm_list': alarm_page.page(current_page).object_list}
...
def device_photo_list(*args, **kwargs):
    ...
    current_page = kwargs.get('current_page', 1)
    page_size = kwargs.get('page_size', 10)
    device_page = Paginator(device_model.DevicePhoto.objects.filter(device__id=Hasher.to_object_pk(_hash)).order_by('-id'), page_size)
    if device_page.page_range.start <= current_page < device_page.page_range.stop:
        return {'device_page': device_page, 'device_photo_list': device_page.page(current_page).object_list}
...
def archives_group(*args, **kwargs):
    ...
    current_page = kwargs.get('current_page', 1)
    page_size = kwargs.get('page_size', 10)
    archives_group_page = Paginator(archives_model.ArchivesGroup.objects.order_by('id'), page_size)
    if archives_group_page.page_range.start <= current_page < archives_group_page.page_range.stop:
        return {'archives_group_page': archives_group_page, 'archives_list': archives_group_page.page(current_page).object_list}
...
def archives_personnel(*args, **kwargs):
    ...
    page_size = kwargs.get('page_size', 10)
    if _hash:
        personnel_page = Paginator(archives_model.Personnel.objects.filter(archives_group_id=Hasher.to_object_pk(_hash)).order_by('-id'), page_size)
    else:
