...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        queryset = monitor_models.ArchivesPersonnel.objects.filter(
        ...
        for query in queryset:
            ...
            logger.info(
                "data: {} Cluster result: {} QueryIdValue: {}".format(
                    datetime.datetime.now(), results, query.id
                )
            )
            if results is not None:
                ...
                query_list = list(
                    device_models.DevicePhoto.objects.in_bulk(check_id_list).values()
                )  # 对未归类的数据进行归类处理
