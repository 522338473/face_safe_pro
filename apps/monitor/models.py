...
class Monitor(BaseModel):
    ...
    personnel_types = fields.ForeignKey(
        to="PersonnelType",
        null=True,
        blank=True,
    ...
    types = fields.IntegerField(
        choices=constant.PERSON_TYPES, default=0, verbose_name="人员分类"
    )
    photo = fields.ImageField(
        drag=True,
        action="/f_upload/monitor/",
        null=True,
        blank=True,
        max_length=128,
        verbose_name="照片",
    ...
    id_type = fields.IntegerField(
        choices=constant.TYPES, default=1, verbose_name="证件类型"
    )
    ...
    source = fields.IntegerField(
        choices=constant.SOURCE, default=1, verbose_name="数据来源"
    )
    right_control = fields.IntegerField(
        choices=constant.CONTROL, default=2, verbose_name="预警范围"
    )
...
class VehicleMonitor(BaseModel):
    ...
    types = fields.IntegerField(
        choices=constant.VEHICLE_TYPES, default=1, verbose_name="车辆分类"
    )
    name = fields.CharField(
        null=True,
        blank=True,
...
class ArchivesPersonnel(BaseModel):
    ...
    def __str__(self):
        return (
            "_".join([self.library.name, self.name])
            if self.library
            else "_".join([self.name])
        )
...
class PhotoCluster(BaseModel):
    ...
    archives_personnel = fields.ForeignKey(
        to="ArchivesPersonnel",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        verbose_name="图片所属人员",
    ...
    device_address = fields.AMapField(
        max_length=128, null=True, blank=True, pick_type="address", verbose_name="设备地址"
    )
    device_geo = fields.AMapField(
        max_length=32, null=True, blank=True, verbose_name="经纬度"
    )
    ...
    device_head_path = fields.ImageField(
        drag=True,
    ...
    device_body_path = fields.ImageField(
        drag=True,
        action="/f_upload/snap/",
        null=True,
        blank=True,
    ...
    device_back_path = fields.ImageField(
        drag=True,
        action="/f_upload/snap/",
        null=True,
        blank=True,
