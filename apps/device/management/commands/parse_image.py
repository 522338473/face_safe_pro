...
class RedisQueue:
    ...
    def callback(self, response):
        ...
        if DeviceInfo.objects.filter(ip=device_ip, delete_at__isnull=True).exists():
            ...
            if face_pic_time:
                self.parse_mq_device_image(device_ip=device_ip, face_pic_time=face_pic_time, face_data=face_data,
                                           human_data=human_data, face_b64=face_b64, human_b64=human_b64, face_panorama_b64=face_panorama_b64, archive_id=archive_id, similarity=similarity)

            elif panorama_b64 and plate_char:
                self.parse_mq_vehicle_image(device_ip=device_ip, vehicle_pic_time=vehicle_pic_time, vehicle_type=vehicle_type,
                                            vehicle_color=vehicle_color, plate_char=plate_char,
                                            panorama_b64=panorama_b64, vehicle_b64=vehicle_b64, plate_b64=plate_b64)

            elif panorama_b64 and vehicle_type == "摩托车-非机动车":
                self.parse_mq_motor_image(device_ip=device_ip, vehicle_pic_time=vehicle_pic_time,
                                          vehicle_type=vehicle_type, panorama_b64=panorama_b64, vehicle_b64=vehicle_b64)

    @staticmethod
    def parse_mq_device_image(device_ip, face_pic_time, face_data, human_data, face_b64, human_b64, face_panorama_b64, archive_id=None, similarity=None):
        """人脸抓拍解析"""
        ...
        res = DevicePhoto.objects.create(
            device=query, take_photo_time=face_pic_time,
            ...
            human_data=json.dumps(human_data, ensure_ascii=False) if human_data else "{}", face_data=json.dumps(face_data, ensure_ascii=False) if face_data else "{}"
        )
        # 将人脸照片的路径丢入redis数据库
        image_list.append([
            res.hash,
        ...
        if archive_id:  # 归档预警
            try:
                AccessDiscover.objects.create(target_id=Hasher.to_object_pk(archive_id), record_id=res.id, similarity=similarity)
            except Exception as e:
                pass

    @staticmethod
    def parse_mq_vehicle_image(device_ip, vehicle_pic_time, vehicle_type, vehicle_color, plate_char, panorama_b64, vehicle_b64, plate_b64):
        """车辆抓拍解析"""
        vehicle_pic_time = datetime.datetime.fromtimestamp(float(vehicle_pic_time / 1000))
        try:
            query = DeviceInfo.objects.filter(delete_at__isnull=True).get(ip=device_ip)
        except Exception as e:
            raise
        ...
        vehicle_id = Vehicle.objects.create(
            take_photo_time=vehicle_pic_time, types=vehicle_type, color=vehicle_color, plate=plate_char,
            address=query.address, geo=query.geo,
        ...
        if VehicleMonitor.objects.filter(delete_at__isnull=True, plate=plate_char).exists() and vehicle_id:
            instance = VehicleMonitorDiscover.objects.create(target_id=VehicleMonitor.objects.filter(delete_at__isnull=True).get(plate=plate_char).pk,
                                                             record_id=vehicle_id.id)

    @staticmethod
    def parse_mq_motor_image(device_ip, vehicle_pic_time, vehicle_type, panorama_b64, vehicle_b64):
        """非机动车抓拍解析"""
        vehicle_pic_time = datetime.datetime.fromtimestamp(float(vehicle_pic_time / 1000))
        try:
            query = DeviceInfo.objects.filter(delete_at__isnull=True).get(ip=device_ip)
        ...
        Motor.objects.create(
            take_photo_time=vehicle_pic_time, types=vehicle_type, address=query.address, geo=query.geo,
